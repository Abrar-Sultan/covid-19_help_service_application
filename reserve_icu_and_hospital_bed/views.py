...
def see_reserved_hospital_bed(request):
    ...
    if not request.user.is_authenticated:
        ...
        return render(request, 'See_Reserved_Hospital_Bed.html',
                      {'see_reserved_hospital_bed_ver': see_reserved_hospital_bed_ver})
...
def thank_you(request):
    ...
    if request.method == "POST":
        ...
        ins = BookingICU(user_name=request.user.username, email=request.user.email, contact_number=contact_number,
                         patient_age=patient_age, patient_name=patient_name, patient_blood_group=patient_blood_group,
                         bkash_number=bkash_number, bkash_transaction_id=bkash_transaction_id,
                         hospital_name=hospital_name, icu_type=icu_type, price=price, is_available=is_available,
                         previous_id=global_veriable)
        ins.save()
...
def thank_you_for_hospital_bed(request):
    ...
    if request.method == "POST":
        ...
        is_available = request.POST['Is_Available']
        ins = BookingHospitalBed(user_name=request.user.username, email=request.user.email,
                                 contact_number=contact_number, available_ac=available_ac, patient_age=patient_age,
                                 patient_name=patient_name, patient_blood_group=patient_blood_group,
                                 bkash_number=bkash_number, bkash_transaction_id=bkash_transaction_id,
                                 hospital_name=hospital_name, bed_number=bed_number, price_per_day=price_per_day,
                                 is_available=is_available, previous_id=global_veriable2)
        ins.save()
